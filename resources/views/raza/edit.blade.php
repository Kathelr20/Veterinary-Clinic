    ...
    <div class="container">
        ...
        <form action="{{ route('raza.update', $raza->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="raza" id="raza" value="{{ $raza->raza }}" class="form-control" required>
            </div>
            <div class="mb-3">
                ...
                <select name="tipo_mascota_id" id="tipo_mascota_id" class="form-select" required>
                    @foreach($tipo_mascotas as $tipo_mascota)
                        <option value="{{ $tipo_mascota->id }}" {{ $tipo_mascota->id == $raza->tipo_mascota_id ? 'selected' : '' }}>{{ $tipo_mascota->mascota }}</option>
                    @endforeach
                </select>
            </div>
