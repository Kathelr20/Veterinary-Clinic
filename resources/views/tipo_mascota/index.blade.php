...
<body>
    ...
    <div class="container mt-4">
        <h1 class="mb-4">Tipos de Mascotas</h1>

        <!-- Botón para agregar nuevo tipo de mascota -->
        <a href="{{ route('tipo_mascota.create') }}" class="btn btn-primary mb-4">
            <i class="fas fa-plus"></i> Nuevo Tipo de Mascota
        </a>

        <div class="row">
            @foreach ($tipo_mascota as $tipo)
                <div class="col-md-4">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title">ID: {{ $tipo->id }}</h5>
                            <p><strong>Mascota:</strong> {{ $tipo->mascota }}</p>
                            <!-- Botones de acción -->
                            <div class="d-flex justify-content-between">
                                <a href="{{ route('tipo_mascota.edit', ['tipo_mascotum' => $tipo->id]) }}" class="btn btn-warning btn-sm">
                                    <i class="fas fa-edit"></i> Editar
                                </a>
                                <form action="{{ route('tipo_mascota.destroy', ['tipo_mascotum' => $tipo->id]) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
